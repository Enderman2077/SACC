...
if __name__ == '__main__':
    ...
    continue_epoch = 10
    model_dir = root + 'model/2023-08-15_12-09-09/'
    model_path = model_dir + 'model_' + str(continue_epoch) + '.pth'
    ...
    for epoch in range(EPOCHS):
        ...
        while i < len(train_data):
            batch = get_batch(train_data, train_ap, i, BATCH_SIZE)
            i += BATCH_SIZE
        ...
        while i < len(val_data):
            batch = get_batch(val_data, val_ap, i, BATCH_SIZE)
            i += BATCH_SIZE
        ...
        print('[Epoch: %3d/%3d] Training Loss: %.4f, Validation Loss: %.4f,'
              ...
              % (continue_epoch + epoch + 1, continue_epoch + EPOCHS, train_loss_[epoch], val_loss_[epoch],
                 train_acc_[epoch], val_acc_[epoch], end_time - start_time))
        torch.save(model.state_dict(), model_dir + '/model_%d.pth' % (continue_epoch + epoch + 1))
        '''
        if (epoch + 1) % 5 == 0:
            torch.save(model.state_dict(), model_dir + '/model_%d.pth' % (continue_epoch + epoch + 1))
