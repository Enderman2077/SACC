...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    # full attention
    pad_attn_mask = (torch.sum(seq_k == zero, dim = 2) > 0).unsqueeze(1)
    # local pattern
    lp = local_pattern(batch_size, len_k).cuda()

    pad_attn_mask = pad_attn_mask.expand(batch_size, len_q, len_k)
    final_mask = pad_attn_mask + lp

    return final_mask # [batch_size, len_q, len_k]
...
class SparseTransformer(nn.Module):
    ...
    def forward(self, x): # x: [batch_size, src_len, d_model]
